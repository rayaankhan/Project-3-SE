...
def handle_all_casinos():
    print("hello")
    # Forward the payment request to the payment backend
    payment_response = requests.post('http://localhost:5001/all_casinos', json=request.json,headers={'Content-Type': 'application/json'})
    print(payment_response.json())
    return payment_response.json(), payment_response.status_code
...
def handle_check_subscription():
    print("hello")
    # Forward the payment request to the payment backend
    payment_response = requests.post('http://localhost:5001/check_subscription', json=request.json,headers={'Content-Type': 'application/json'})
    print(payment_response.json())
    return payment_response.json(), payment_response.status_code
...
def handle_wallet_addBalance():
    print("hello")
    # Forward the payment request to the payment backend
    payment_response = requests.post('http://localhost:8080/wallet/addBalance', json=request.json,headers={'Content-Type': 'application/json'})
    print(payment_response.json())
    return payment_response.json(), payment_response.status_code
